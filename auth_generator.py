            ...
            if file_type == 'db':
                ...
                try:
                    for host_key in data_controller.host_key_db(app_name):
                        ...
                        add_str = cookie_str_join(extracted_data)
                        if add_str != '':
                            cookie_str += '[' + host_key + ']\n'
                            cookie_str += add_str + '\n'
                # None Type Error 방지.
                # data_controller에서 app_name을 찾지 못하면 모든 cookie값을 가져온다.
                    ...
                    cookie_str += cookie_generate(extracted_data)
                cookie_list.append(cookie_generate(file_name, cookie_str))
                conn.close()
...
def cookie_str_join(name_value):
    cookie_str = ''
    for s in name_value:
        cookie_str += s[0] + '=' + s[1] + '\n'
    return cookie_str
...
def cookie_generate(file_name, cookie_str):
